...
class RewardGrapher:
    ...
    def readRewards(self, filename: str) -> list[float]:
        with open(filename, 'r') as f:
            rewards = [float(line.strip()) for line in f]
        return rewards
    
    def calculateSlope(self, rewards: list[float]) -> tuple[float, float]:
    ...
    def plotRewards(self, rewards: list[float], slope: float, intercept: float, label: str, ax: Axes) -> None:
    ...
    def plotMultipleRewards(self, ax: Axes) -> None:
        for filename in self.filenames:
            rewards = self.readRewards(filename)
            slope, intercept = self.calculateSlope(rewards)
            try:
                label = os.path.basename(os.path.dirname(filename)) or os.path.basename(filename)
            except Exception:
                label = "Run"
            self.plotRewards(rewards, slope, intercept, label, ax)
    ...
    def run(self, canvas: Any) -> FigureCanvasTkAgg:
        ...
        if len(self.filenames) == 1:
            rewards = self.readRewards(self.filenames[0])
            slope, intercept = self.calculateSlope(rewards)
            self.plotRewards(rewards, slope, intercept, 'Single', ax)
            ax.set_xlabel('Episode')
            ax.set_ylabel('Cumulative Reward')
            ax.set_title(f'Rewards over Episodes\nSlope: {slope:.2f}')
            ax.legend()
        else:
            self.plotMultipleRewards(ax)

        # Embed the plot in the Tkinter canvas
        canvasAgg = FigureCanvasTkAgg(fig, master=canvas)
        canvasAgg.draw()
        canvasAgg.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, expand=1)
