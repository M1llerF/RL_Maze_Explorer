...
class ActionRegistry:
    ...
    def execute(
        ...
        envReward, discountedReward, duration, done = self.executeOption(option, context, gamma=gamma)
        return StepResult(
            reward=envReward,
            done=done,
    ...
    def executeOption(
        ...
        """
        Run an option to termination. Returns
        (cumulative_environment_reward, cumulative_discounted_reward, duration, done).
        ...
        while not option.shouldTerminate(context, duration) and not done:
            ...
            step_reward = result.reward + option.intrinsicReward(context, result)
            env_reward += step_reward
            discounted_reward += (gamma ** duration) * step_reward
            duration += 1
            done = result.done
            if done:
                break
        return env_reward, discounted_reward, duration, done
