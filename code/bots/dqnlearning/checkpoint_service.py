...
class DQNCheckpointService:
    ...
    def save(self, bot: Any) -> None:
        ...
        if isinstance(bot.agent, HierarchicalDqnAgent):
            ...
            manualEpsilon = bot.agent.highLevel.manualEpsilonOverride
        else:
            manualEpsilon = bot.agent.manualEpsilonOverride
        if manualEpsilon is not None:
            extraState["manual_epsilon"] = float(manualEpsilon)
    ...
    def load(self, bot: Any) -> bool:
        ...
        savedEpsilon = (checkpoint.get("extra") or {}).get("manual_epsilon")
        if savedEpsilon is not None:
            bot.agent.setManualEpsilon(float(savedEpsilon))
        ...
        self._log(
            ...
            replay_size=int(len(bot.agent.replay)),
        )
        return True

    def _tryLoadPrimitiveBootstrapFallback(self, bot: Any) -> bool:
        if bot.usesHierarchicalPolicy or bot.usesMacroOnlyPolicy or bool(getattr(bot.config, "useMacroActions", False)):
            return False
